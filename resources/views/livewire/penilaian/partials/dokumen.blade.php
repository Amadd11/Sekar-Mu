<div
    ...
    class="space-y-6"
>
    <!-- 1. Header & Counter Arsip Dokumen Bukti (Matching Dokumen Anggota) -->
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div class="space-y-1">
                <div class="flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <span>•</span>
                    <span class="font-bold text-slate-700 font-mono">{{ $dokumenTotalFilesAll }} Berkas ({{ $dokumenTotalButirWithFilesAll }} Butir Berlampiran)</span>
                </div>
                <h2 class="font-display text-base sm:text-lg font-bold text-slate-900 tracking-tight">
                    Arsip Dokumen Bukti Evaluasi Diri
                </h2>
                <p class="text-xs text-slate-500">
                    Telaah dokumen bukti dukung butir standar yang diunggah oleh pemohon KEPK untuk pertimbangan penilaian akreditasi.
                </p>
            </div>
            ...
            <div class="flex items-center gap-2 shrink-0">
                <button
                    ...
                    class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-white bg-primary-700 hover:bg-primary-600 active:bg-primary-800 transition shadow-2xs cursor-pointer"
                >
                    <span class="material-symbols-outlined text-[16px]" style="font-variation-settings: 'FILL' 1;">fact_check</span>
                    <span>Beri Nilai di Tab Penilaian</span>
                </button>
            </div>
        </div>
    </div>

    <!-- 2. Search & Status Filter Bar (Matching Dokumen Anggota) -->
    <div class="bg-white rounded-2xl border border-slate-200/80 p-3 shadow-2xs flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div class="relative w-full sm:w-80">
            ...
            <input
                type="text"
                wire:model.live.debounce.300ms="dokumenSearch"
        ...
        <div class="flex items-center gap-1.5 w-full sm:w-auto overflow-x-auto text-xs">
            ...
            <button
                type="button"
                wire:click="setDokumenFilterStatus('all')"
                class="px-2.5 py-1 rounded-lg font-medium transition cursor-pointer {{ $dokumenFilterStatus === 'all' ? 'bg-slate-900 text-white font-bold' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}"
            >
                Semua
            </button>
            <button
                type="button"
                wire:click="setDokumenFilterStatus('lengkap')"
                class="px-2.5 py-1 rounded-lg font-medium transition cursor-pointer flex items-center gap-1 {{ $dokumenFilterStatus === 'lengkap' ? 'bg-emerald-600 text-white font-bold' : 'bg-emerald-50 text-emerald-800 hover:bg-emerald-100 border border-emerald-200/60' }}"
            >
                <span class="w-1.5 h-1.5 rounded-full {{ $dokumenFilterStatus === 'lengkap' ? 'bg-white' : 'bg-emerald-500' }}"></span>
                <span>Lengkap (≥2)</span>
            </button>
            <button
                type="button"
                wire:click="setDokumenFilterStatus('belum_lengkap')"
                class="px-2.5 py-1 rounded-lg font-medium transition cursor-pointer flex items-center gap-1 {{ $dokumenFilterStatus === 'belum_lengkap' ? 'bg-amber-500 text-white font-bold' : 'bg-amber-50 text-amber-800 hover:bg-amber-100 border border-amber-200/60' }}"
            >
                <span class="w-1.5 h-1.5 rounded-full {{ $dokumenFilterStatus === 'belum_lengkap' ? 'bg-white' : 'bg-amber-500' }}"></span>
                <span>1 Berkas</span>
            </button>
        </div>
    </div>

    <!-- 3. Section Segmented Tabs (Bagian A - E, Matching Dokumen Anggota) -->
    <div class="bg-slate-100/80 p-1.5 rounded-2xl border border-slate-200/80 grid grid-cols-2 sm:grid-cols-5 gap-1.5">
        @foreach ($bagianList as $b)
            @php
                $secFileCount = $dokumenFileCountsPerBagian[$b->kode] ?? 0;
                $secButirCount = $dokumenButirCountsPerBagian[$b->kode] ?? 0;
            ...
            <button
                ...
                class="rounded-xl px-3 py-2.5 text-left transition-all flex flex-col justify-between gap-1 cursor-pointer {{ $isActive ? 'bg-white text-slate-900 shadow-xs border border-slate-200/80 ring-1 ring-primary-500/20' : 'text-slate-600 hover:text-slate-900 hover:bg-white/50' }}"
            >
    ...
    <!-- 4. Active Section High-Density Butir Table (1 Row per Butir, Matching Dokumen Anggota) -->
    @php
        $secItems = $dokumenActiveSectionData['items'] ?? [];
        $secTotalFiles = $dokumenActiveSectionData['total_files'] ?? 0;
    @endphp
        ...
        <div class="px-5 py-3.5 bg-slate-50/80 border-b border-slate-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <h3 class="font-display text-sm font-bold text-slate-900 leading-snug">
                        Bagian {{ $activeBagian->kode }}: {{ $activeBagian->nama }}
                    </h3>
                    <p class="text-[11px] text-slate-500">
                        {{ count($secItems) }} butir dengan total {{ $secTotalFiles }} berkas bukti dukung terlampir.
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2">
                <button
                    type="button"
                    wire:click="switchTab('penilaian')"
                    class="inline-flex items-center gap-1 px-3 py-1.5 bg-white hover:bg-slate-100 text-primary-700 text-xs font-bold rounded-xl border border-slate-200 transition shadow-2xs cursor-pointer"
                >
                    <span>Ke Penilaian Bagian {{ $activeBagian->kode }}</span>
                ...
                <table class="w-full text-left border-collapse text-xs min-w-[760px]">
                    <thead>
                        <tr class="border-b border-slate-100 text-[11px] font-semibold text-slate-400 uppercase tracking-wider bg-white">
                            ...
                            <th class="py-2.5 px-4 w-44 text-center">Aksi Asesor</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-slate-50/70 transition">
                                ...
                                <td class="py-3 px-4 align-middle">
                                    <div class="space-y-0.5">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span class="text-[10px] text-slate-400">• {{ $item['kelompok_nama'] }}</span>
                                        </div>
                                ...
                                <td class="py-3 px-4 text-center align-middle">
                                    <div class="flex items-center justify-center gap-1.5">
                                        <button
                                            type="button"
                                            @click="openModal({{ json_encode($item, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) }})"
        ...
        @else
            ...
            <div class="p-10 text-center text-slate-400 space-y-2">
                ...
                <p class="text-xs font-semibold text-slate-600">
                    @if ($dokumenSearch !== '')
                        Tidak ada butir berlampiran yang cocok dengan pencarian "{{ $dokumenSearch }}" pada Bagian {{ $activeBagian->kode }}.
                    @else
                        Belum ada berkas bukti dukung yang diunggah pemohon pada Bagian {{ $activeBagian->kode }}.
                    @endif
                </p>
                <p class="text-[11px] text-slate-400">
                    Pilih tab bagian lain di atas untuk menelaah dokumen bukti dukung standar akreditasi KEPK.
                </p>
            </div>
        @endif
    </div>

    <!-- 5. Simple Clean File List Modal (Teleport to body, matching Dokumen Anggota) -->
    <template x-teleport="body">
        ...
        >
            ...
            >
                ...
                <!-- Modal Subheader: SK / Bukti & Catatan KEPK -->
                <div class="px-6 py-3 bg-slate-50/50 border-b border-slate-100 text-xs space-y-2" x-show="selectedItem && (selectedItem.bukti || selectedItem.catatan)">
                    <template x-if="selectedItem && selectedItem.bukti">
                        <div class="text-slate-700 bg-white p-2.5 rounded-xl border border-slate-200/80 flex items-center gap-2">
                            <span class="material-symbols-outlined text-primary-700 text-[16px]">verified</span>
                            <span><strong>No. SK / Bukti Dukung:</strong> <span x-text="selectedItem.bukti"></span></span>
                        </div>
                    </template>
                    <template x-if="selectedItem && selectedItem.catatan">
                        <div class="text-slate-600 bg-white p-2.5 rounded-xl border border-slate-200/80">
                            <span class="font-bold text-slate-700">Uraian / Penjelasan KEPK:</span>
                            <span x-text="selectedItem.catatan"></span>
                        </div>
                    </template>
                </div>
                ...
                <div class="px-6 py-3.5 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-3 text-xs">
                    <button
                        ...
                        wire:click="switchTab('penilaian')"
                        @click="closeModal()"
                        class="inline-flex items-center gap-1.5 text-primary-700 hover:text-primary-800 font-bold cursor-pointer"
                    >
                        <span>Beri Nilai di Tab Penilaian</span>
