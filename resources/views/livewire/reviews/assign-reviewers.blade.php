<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="bg-white border border-slate-200/90 rounded-xl p-5 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-lg font-bold text-slate-900">
                Penugasan Penelaah Etik (Assign Reviewers)
            </h1>
            <p class="text-xs text-slate-500 mt-0.5">
                Pengajuan: {{ $application->information->name ?? 'Permohonan Etik' }}
            </p>
        </div>

        <div>
            <a href="{{ route('applications.show', $application) }}" class="btn-outline btn-sm text-xs" wire:navigate>
    ...
    <div class="bg-white border border-slate-200/90 rounded-xl p-6 shadow-2xs space-y-6">
        <div>
            <h2 class="text-sm font-bold text-slate-900">Pilih Penelaah Etik Independen</h2>
            <p class="text-xs text-slate-500 mt-0.5">
                Pilih satu atau lebih penelaah etik terdaftar untuk meninjau kelayakan protokol ini. Status pengajuan akan otomatis beralih menjadi <strong>Sedang Ditelaah (Under Review)</strong>.
            </p>
        </div>
        ...
        <form wire:submit="save" class="space-y-4">
            ...
            <div class="pt-4 flex justify-end">
                ...
                >
                    <span wire:loading.remove wire:target="save">💾 Simpan Penugasan Penelaah</span>
