<x-app-layout>
    @php
        $user = auth()->user();
        $suratPengajuan = \App\Models\SuratPengajuan::with(['kepk.institusi', 'formulirAplikasi', 'profilKepk', 'jawabanEvaluasi'])->where('user_id', $user->id)->latest()->first();
        if (!$suratPengajuan && ($user->isAdmin() || $user->isReviewer())) {
            $suratPengajuan = \App\Models\SuratPengajuan::with(['kepk.institusi', 'formulirAplikasi', 'profilKepk', 'jawabanEvaluasi'])->latest()->first();
        }

        $evaluasiService = app(\App\Services\EvaluasiDiriService::class);
        $progress = $suratPengajuan ? $evaluasiService->calculateProgress($suratPengajuan) : [];
        $rekapSkor = $suratPengajuan ? $evaluasiService->calculateScoreSummary($suratPengajuan) : ['total' => 0, 'skor_a' => 0, 'skor_b' => 0, 'skor_c' => 0];
    ...
    <div class="space-y-5 max-w-7xl mx-auto">
        ...
        <div class="bg-white border border-slate-200/90 rounded-xl p-5 shadow-2xs space-y-4">
            <div class="flex items-center justify-between pb-2 border-b border-slate-100">
                ...
                @if ($suratPengajuan)
                    <a href="{{ route('pengajuan.evaluasi-diri', $suratPengajuan) }}" class="text-xs text-teal-700 font-semibold hover:underline" wire:navigate>
            ...
            <div class="space-y-3.5 text-xs">
                ...
                @foreach ($sectionsMeta as $code => $meta)
                    @php
                        $secData = $progress[$code] ?? ['terjawab' => 0, 'total' => $meta['default_total'], 'persentase' => 0];
                    @endphp
                    <x-section-progress-row
                        :name="$meta['name']"
                        :answered="$secData['terjawab']"
                        :total="$secData['total']"
                        :percentage="$secData['persentase']"
                    />
                @endforeach
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div class="bg-white border border-slate-200/90 rounded-xl p-5 shadow-2xs flex flex-col">
                ...
                <div class="flex-1 flex flex-col items-center justify-center py-6 text-center text-slate-400">
                    @if ($rekapSkor['total'] > 0)
                        <div class="text-2xl font-black text-teal-700 mb-1">
                            {{ $rekapSkor['skor_a'] >= 100 ? 'Akreditasi Utama (A)' : ($rekapSkor['skor_a'] >= 50 ? 'Akreditasi Madya (B)' : 'Dalam Proses Evaluasi') }}
                        </div>
                        <p class="text-xs text-slate-500">
                            {{ $rekapSkor['total'] }} dari 155 butir telah dievaluasi.
                        </p>
                    @else
        ...
        <div class="bg-white border border-slate-200/90 rounded-xl p-5 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div>
                @if ($suratPengajuan)
                    <a href="{{ route('pengajuan.evaluasi-diri', $suratPengajuan) }}" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 transition shadow-2xs" wire:navigate>
                        <span>💾</span>
                        <span>Mulai Pengisian Evaluasi Diri</span>
                    </a>
                @else
                    <a href="{{ route('pengajuan.create') }}" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 transition shadow-2xs" wire:navigate>
                        <span>💾</span>
